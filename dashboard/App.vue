  ...
  <div id="app" class="h-screen w-screen flex flex-col bg-white dark:bg-black text-black dark:text-white font-mono transition-colors duration-300">
      ...
      <div class="ticker-content whitespace-nowrap">
        <template v-if="activityFeed.length > 0">
          ...
          <template v-for="n in 2" :key="`loop-${n}`">
            <span
              v-for="(activity, i) in activityFeed"
              :key="`${n}-${i}`"
              class="inline-block"
    ...
    <header class="flex-shrink-0 px-4 py-4 flex items-center justify-between">
      <div class="flex items-center gap-4">
        <h1 class="text-base uppercase tracking-widest">
          SCRAP MONITOR
        </h1>
        <div v-if="isConnected" class="flex items-center gap-3 text-base">
          <div class="w-4 h-4 rounded-full animate-pulse" style="background-color: #10b981; box-shadow: 0 0 12px #10b981, 0 0 4px #10b981 inset;" />
          <span>LIVE</span>
        </div>
        <div v-else class="flex items-center gap-3 text-base opacity-50">
          <div class="w-4 h-4 rounded-full" style="background-color: #ef4444; box-shadow: 0 0 8px #ef4444;" />
          <span>OFFLINE</span>
        </div>
      </div>
    ...
    <main class="flex-1 overflow-hidden">
      <CurrentScrap
        :scrap="currentScrap"
        :is-pulsing="currentScrap && recentlyUpdated.has(currentScrap.id)"
      />
    </main>
  </div>
</template>
