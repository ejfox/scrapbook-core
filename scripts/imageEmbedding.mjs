...
// Add rate limiter for Nomic API
const nomicLimiter = new Bottleneck({
...
function extractArenaImages(scrap) {
  ...
  logger.debug('Arena block metadata:', JSON.stringify(scrap.metadata, null, 2));
  return [];
}
...
async function extractGithubImages(scrap) {
  try {
    ...
    $('img').each((i, elem) => {
      ...
      if (src && !src.includes('badge')) { // Skip badges
        images.push(src);
      }
    });
