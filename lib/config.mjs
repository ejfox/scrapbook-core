...
import { validateEnvironment } from './validateEnvironment.mjs'
...
function validateConfig(config) {
  ...
  if (envResult.errors.length) {
    throw new Error(
      `Environment validation failed: ${envResult.errors.join('; ')}`,
    )
  }
  if (envResult.warnings.length) {
    envResult.warnings.forEach((w) => console.warn(`[config] ${w}`))
  }
}
