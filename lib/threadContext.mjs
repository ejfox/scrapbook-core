...
export function formatThreadContext(relatedScraps) {
  ...
  const formatted = relatedScraps
   ...
   Location: ${location} | Type: ${contentType}
   Shared tags: ${tags}
   Summary: ${snippet}${snippet.length >= 200 ? '...' : ''}`
    }).join('\n\n')

  return `
RECENTLY SAVED RELATED BOOKMARKS (with similar tags):

${formatted}

NOTE: These bookmarks share tags with the current content but may or may not be actually related. They could be:
